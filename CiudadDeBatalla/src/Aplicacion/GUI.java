...
public class GUI implements KeyListener  {
	...
	private void initialize() {
		...
		lblPlayer.setIcon(new ImageIcon(new ImageIcon(partida.getJugador().getPath()).getImage().getScaledInstance(tamanio_celda, tamanio_celda, Image.SCALE_SMOOTH)));
		...
		for(int i=0;i<4;i++){
			...
			lblEnemigo[i].setIcon(new ImageIcon(new ImageIcon(partida.getEnemigo(i).getPath()).getImage().getScaledInstance(tamanio_celda, tamanio_celda, Image.SCALE_SMOOTH)));
	...
	public void crearMapa(){			//rpueba
		gameObject[][] mapa=partida.getMapa();
		for(int f=0;f<mapa.length;f++){
			for(int c=0;c<mapa.length;c++){	
				MapaVirtual[f][c] = new JLabel(new ImageIcon(new ImageIcon(mapa[f][c].getPath()).getImage().getScaledInstance(tamanio_celda, tamanio_celda, Image.SCALE_SMOOTH)));
